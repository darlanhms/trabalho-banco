...
export const createCarga = async (req:Req, res:Res) => {
    const { dataEntrada, dataEntrega, peso, largura, altura, comprimento, status, enderecoCarga, cliente_id } = req.body;
    ...
    if (dataEntrada && dataEntrega && peso && largura && altura && comprimento && status && cidade && estado && logradouro) {
        pool.query('INSERT INTO carga(dataEntrada, dataEntrega, peso, largura, altura, comprimento, status, cliente_id) VALUES ($1::text, $2::text, $3::text, $4::text, $5::text, $6::text, $7::text, $8::int) RETURNING *', [dataEntrada, dataEntrega, peso, largura, altura, comprimento, status, cliente_id])
        .then(carga => {
